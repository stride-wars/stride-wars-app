...
func (fq *FriendshipQuery) QueryUsers() *UserQuery {
...
func (fq *FriendshipQuery) QueryFriends() *UserQuery {
	query := (&UserClient{config: fq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := fq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (fq *FriendshipQuery) Clone() *FriendshipQuery {
	...
	return &FriendshipQuery{
		...
		withFriends: fq.withFriends.Clone(),
...
func (fq *FriendshipQuery) WithUsers(opts ...func(*UserQuery)) *FriendshipQuery {
	query := (&UserClient{config: fq.config}).Query()
...
func (fq *FriendshipQuery) WithFriends(opts ...func(*UserQuery)) *FriendshipQuery {
	query := (&UserClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withFriends = query
	return fq
}
...
//		GroupBy(friendship.FieldUserID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (fq *FriendshipQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Friendship, error) {
	...
	if query := fq.withUsers; query != nil {
		if err := fq.loadUsers(ctx, query, nodes, nil,
			func(n *Friendship, e *User) { n.Edges.Users = e }); err != nil {
			return nil, err
		}
	}
	if query := fq.withFriends; query != nil {
		if err := fq.loadFriends(ctx, query, nodes, nil,
			func(n *Friendship, e *User) { n.Edges.Friends = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FriendshipQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Friendship, init func(*Friendship), assign func(*Friendship, *User)) error {
...
func (fq *FriendshipQuery) loadFriends(ctx context.Context, query *UserQuery, nodes []*Friendship, init func(*Friendship), assign func(*Friendship, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Friendship)
	for i := range nodes {
		fk := nodes[i].FriendID
