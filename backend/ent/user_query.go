...
func (uq *UserQuery) QueryFriendship() *FriendshipQuery {
...
// QueryHexinfluence chains the current query on the "hexinfluence" edge.
func (uq *UserQuery) QueryHexinfluence() *HexInfluenceQuery {
	query := (&HexInfluenceClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithFriendship(opts ...func(*FriendshipQuery)) *UserQuery {
	query := (&FriendshipClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withFriendship = query
	return uq
}

// WithHexinfluence tells the query-builder to eager-load the nodes that are connected to
// the "hexinfluence" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithHexinfluence(opts ...func(*HexInfluenceQuery)) *UserQuery {
	query := (&HexInfluenceClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withHexinfluence = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withFriendship; query != nil {
		if err := uq.loadFriendship(ctx, query, nodes,
			func(n *User) { n.Edges.Friendship = []*Friendship{} },
			func(n *User, e *Friendship) { n.Edges.Friendship = append(n.Edges.Friendship, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withHexinfluence; query != nil {
		if err := uq.loadHexinfluence(ctx, query, nodes,
			func(n *User) { n.Edges.Hexinfluence = []*HexInfluence{} },
			func(n *User, e *HexInfluence) { n.Edges.Hexinfluence = append(n.Edges.Hexinfluence, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadFriendship(ctx context.Context, query *FriendshipQuery, nodes []*User, init func(*User), assign func(*User, *Friendship)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Friendship(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.FriendshipColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (uq *UserQuery) loadHexinfluence(ctx context.Context, query *HexInfluenceQuery, nodes []*User, init func(*User), assign func(*User, *HexInfluence)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(hexinfluence.FieldUserID)
	}
	query.Where(predicate.HexInfluence(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.HexinfluenceColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
