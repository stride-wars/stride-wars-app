...
import (
	"context"
	"fmt"
	"stride-wars-app/internal/ent/generated"
)

// The BaseFunc type is an adapter to allow the use of ordinary
// function as Base mutator.
type BaseFunc func(context.Context, *generated.BaseMutation) (generated.Value, error)

// Mutate calls f(ctx, m).
func (f BaseFunc) Mutate(ctx context.Context, m generated.Mutation) (generated.Value, error) {
	if mv, ok := m.(*generated.BaseMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *generated.BaseMutation", m)
}
